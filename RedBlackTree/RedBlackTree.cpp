...
void RedBlackTree::PrintTree(RBTreeNode *node, int depth)
{
	if(depth == 0) 
	{
		sprintf(buffer, "PRINT TREE [KEY %lld]\n", m_key);
		std::cout<<buffer;
	}
	if(node == NULL) return;
...
int	RedBlackTree::init(key_t key, int maxSize, int **ppCount)
{
	#ifdef FILE_DEBUGING
	// fstream 
	char name[256];
	sprintf(name, "RBLOG%d", key);
	fs.open(name, std::ios::out | std::ios::app | std::ios::binary);
	if(fs.is_open())
	{
		fs << "-----ReStart Log-----\n";
	}
	...
	{
		m_header = (RBHeader *)shmat(shmId, 0, 0);
		memset(m_header, 0, sizeof(RBHeader));
	}
...
RBTreeNode* RedBlackTree::find(unsigned long long key)
{
	...
	while (pointer != NULL)
	{
		...
		{
			printf("  [%d](%llx,%lld)\n /         \\\n[%d]         [%d]\n", pointer->index, pointer->value.Key, pointer->value.Value, pointer->left, pointer->right);
...
void RedBlackTree::RightRotate(RBTreeNode *item)
{
	...
	{
		printf("%d %d %d %d\n", parent->index != item->parent, (grandParent!=NULL && grandParent->index != parent->parent) ,parent->left != item->index , (rightChild != NULL && (rightChild->parent != item->index || item->right != rightChild->index)) );
...
void RedBlackTree::LeftRotate(RBTreeNode *item)
{
	...
	{
		printf("%d %d %d %d\n", parent->index != item->parent, (grandParent!=NULL && grandParent->index != parent->parent) ,parent->right != item->index , (leftChild != NULL && (leftChild->parent != item->index || item->left != leftChild->index)) );
...
void RedBlackTree::CheckTree(RBTreeNode *item)
{
	...
	if(item->color == RED && parentNode->color == RED) // 연속으로 Red가 2개 인 경우
	{
		...
		if(grandParent == NULL)
		{
			printf("<<<<<ParentNode Is RED & GrandParent Node Is NULL>>>>>>"); // root == RED !!
			PrintTree();
			return;
		}
...
void RedBlackTree::RemoveDoubleBlack(RBTreeNode *doubleBlack, RBTreeNode *parent)
{
	...
	if(sbling == NULL)
	{
		printf("sbling is NULL [Parent %d][DoubleBlackNode %d]\n", parent->index, doubleBlack==NULL?0:doubleBlack->index);
		#ifdef FILE_DEBUGING
		if(fs.is_open())
		{
			fs << "sbling NULL\n";
